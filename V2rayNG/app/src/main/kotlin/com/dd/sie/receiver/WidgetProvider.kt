...
class WidgetProvider : AppWidgetProvider() {
    ...
    private fun updateWidgetBackground(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray, isRunning: Boolean) {
        ...
        intent.action = com.dd.sie.AppConfig.BROADCAST_ACTION_WIDGET_CLICK
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (com.dd.sie.AppConfig.BROADCAST_ACTION_WIDGET_CLICK == intent.action) {
            if (V2RayServiceManager.v2rayPoint.isRunning) {
                Utils.stopVService(context)
            } else {
                Utils.startVServiceFromToggle(context)
            }
        } else if (com.dd.sie.AppConfig.BROADCAST_ACTION_ACTIVITY == intent.action) {
            AppWidgetManager.getInstance(context)?.let { manager ->
                when (intent.getIntExtra("key", 0)) {
                    com.dd.sie.AppConfig.MSG_STATE_RUNNING, com.dd.sie.AppConfig.MSG_STATE_START_SUCCESS -> {
                        updateWidgetBackground(context, manager, manager.getAppWidgetIds(ComponentName(context, WidgetProvider::class.java)),
                                true)
                    }
                    com.dd.sie.AppConfig.MSG_STATE_NOT_RUNNING, com.dd.sie.AppConfig.MSG_STATE_START_FAILURE, com.dd.sie.AppConfig.MSG_STATE_STOP_SUCCESS -> {
                        updateWidgetBackground(context, manager, manager.getAppWidgetIds(ComponentName(context, WidgetProvider::class.java)),
                                false)
                    }
                }
