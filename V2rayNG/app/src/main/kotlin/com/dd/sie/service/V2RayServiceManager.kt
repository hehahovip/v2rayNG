...
import com.dd.sie.util.V2rayConfigUtil
import go.Seq
...
object V2RayServiceManager {
    ...
    fun startV2Ray(context: Context) {
        ...
        val intent = if (settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_MODE) ?: "VPN" == "VPN") {
    ...
    fun startV2rayPoint() {
        ...
        if (!v2rayPoint.isRunning) {
            ...
            try {
                val mFilter = IntentFilter(com.dd.sie.AppConfig.BROADCAST_ACTION_SERVICE)
            ...
            try {
                v2rayPoint.runLoop(settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_PREFER_IPV6) ?: false)
            } catch (e: Exception) {
                Log.d(ANG_PACKAGE, e.toString())
            }

            if (v2rayPoint.isRunning) {
                MessageUtil.sendMsg2UI(service, com.dd.sie.AppConfig.MSG_STATE_START_SUCCESS, "")
                showNotification()
            } else {
                MessageUtil.sendMsg2UI(service, com.dd.sie.AppConfig.MSG_STATE_START_FAILURE, "")
                cancelNotification()
            }
        }
    }
    ...
    fun stopV2rayPoint() {
        ...
        MessageUtil.sendMsg2UI(service, com.dd.sie.AppConfig.MSG_STATE_STOP_SUCCESS, "")
        cancelNotification()
    ...
    private class ReceiveMessageHandler : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent?.getIntExtra("key", 0)) {
                com.dd.sie.AppConfig.MSG_REGISTER_CLIENT -> {
                    //Logger.e("ReceiveMessageHandler", intent?.getIntExtra("key", 0).toString())
                    if (v2rayPoint.isRunning) {
                        MessageUtil.sendMsg2UI(serviceControl.getService(), com.dd.sie.AppConfig.MSG_STATE_RUNNING, "")
                    } else {
                        MessageUtil.sendMsg2UI(serviceControl.getService(), com.dd.sie.AppConfig.MSG_STATE_NOT_RUNNING, "")
                    }
                }
                com.dd.sie.AppConfig.MSG_UNREGISTER_CLIENT -> {
                    // nothing to do
                }
                com.dd.sie.AppConfig.MSG_STATE_START -> {
                    // nothing to do
                }
                com.dd.sie.AppConfig.MSG_STATE_STOP -> {
                    serviceControl.stopService()
                }
                com.dd.sie.AppConfig.MSG_STATE_RESTART -> {
                    startV2rayPoint()
                }
                com.dd.sie.AppConfig.MSG_MEASURE_DELAY -> {
                    measureV2rayDelay()
                }
            }
    ...
    private fun measureV2rayDelay() {
        GlobalScope.launch(Dispatchers.IO) {
            ...
            MessageUtil.sendMsg2UI(service, com.dd.sie.AppConfig.MSG_MEASURE_DELAY_SUCCESS, result)
        }
    }
    ...
    private fun showNotification() {
        ...
        val stopV2RayIntent = Intent(com.dd.sie.AppConfig.BROADCAST_ACTION_SERVICE)
        stopV2RayIntent.`package` = ANG_PACKAGE
        stopV2RayIntent.putExtra("key", com.dd.sie.AppConfig.MSG_STATE_STOP)
    ...
    private fun startSpeedNotification() {
        if (mSubscription == null &&
                ...
                settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_SPEED_ENABLED) == true) {
