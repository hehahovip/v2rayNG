...
import com.dd.sie.dto.V2rayConfig
import com.dd.sie.dto.ERoutingMode
import com.dd.sie.dto.V2rayConfig.Companion.DEFAULT_NETWORK
import com.dd.sie.dto.V2rayConfig.Companion.HTTP
...
object V2rayConfigUtil {
    ...
    fun getV2rayConfig(context: Context, guid: String): Result {
        try {
            ...
            if (config.configType == com.dd.sie.dto.EConfigType.CUSTOM) {
    ...
    private fun getV2rayNonCustomConfig(context: Context, outbound: V2rayConfig.OutboundBean): Result {
        ...
        v2rayConfig.log.loglevel = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_LOGLEVEL)
                ?: "warning"
        ...
        if (settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_LOCAL_DNS_ENABLED) == true) {
            customLocalDns(v2rayConfig)
        }
        if (settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_SPEED_ENABLED) != true) {
    ...
    private fun inbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            val socksPort = Utils.parseInt(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_SOCKS_PORT), com.dd.sie.AppConfig.PORT_SOCKS.toInt())
            val httpPort = Utils.parseInt(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_HTTP_PORT), com.dd.sie.AppConfig.PORT_HTTP.toInt())

            v2rayConfig.inbounds.forEach { curInbound ->
                if (settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_PROXY_SHARING) != true) {
                    //bind all inbounds to localhost if the user requests
                    curInbound.listen = "127.0.0.1"
                }
            }
            v2rayConfig.inbounds[0].port = socksPort
            val fakedns = settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_FAKE_DNS_ENABLED)
                    ?: false
            val sniffAllTlsAndHttp = settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_SNIFFING_ENABLED, true)
                    ?: true
...
//            }
    ...
    private fun fakedns(v2rayConfig: V2rayConfig) {
        if (settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_FAKE_DNS_ENABLED) == true) {
    ...
    private fun routing(v2rayConfig: V2rayConfig): Boolean {
        try {
            routingUserRule(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_AGENT)
                    ?: "", com.dd.sie.AppConfig.TAG_AGENT, v2rayConfig)
            routingUserRule(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_DIRECT)
                    ?: "", com.dd.sie.AppConfig.TAG_DIRECT, v2rayConfig)
            routingUserRule(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_BLOCKED)
                    ?: "", com.dd.sie.AppConfig.TAG_BLOCKED, v2rayConfig)

            v2rayConfig.routing.domainStrategy = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_ROUTING_DOMAIN_STRATEGY)
                    ?: "IPIfNonMatch"
//            v2rayConfig.routing.domainMatcher = "mph"
            val routingMode = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_ROUTING_MODE) ?: ERoutingMode.GLOBAL_PROXY.value

            // Hardcode googleapis.cn
            val googleapisRoute = V2rayConfig.RoutingBean.RulesBean(
                    type = "field",
                    outboundTag = com.dd.sie.AppConfig.TAG_AGENT,
                    domain = arrayListOf("domain:googleapis.cn")
            )

            when (routingMode) {
                ERoutingMode.BYPASS_LAN.value -> {
                    routingGeo("ip", "private", com.dd.sie.AppConfig.TAG_DIRECT, v2rayConfig)
                }
                ERoutingMode.BYPASS_MAINLAND.value -> {
                    routingGeo("", "cn", com.dd.sie.AppConfig.TAG_DIRECT, v2rayConfig)
                ...
                ERoutingMode.GLOBAL_DIRECT.value -> {
                    val globalDirect = V2rayConfig.RoutingBean.RulesBean(
                        type = "field",
                        outboundTag = com.dd.sie.AppConfig.TAG_DIRECT,
                        port = "0-65535"
                    )
...
//                                    || it.startsWith("full:"))
    ...
    private fun customLocalDns(v2rayConfig: V2rayConfig): Boolean {
        try {
            if (settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_FAKE_DNS_ENABLED) == true) {
                ...
                val proxyDomain = userRule2Domian(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_AGENT)
                        ?: "")
                val directDomain = userRule2Domian(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_DIRECT)
                        ?: "")
            ...
            if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                ...
                val localDnsPort = Utils.parseInt(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_LOCAL_DNS_PORT), com.dd.sie.AppConfig.PORT_LOCAL_DNS.toInt())
    ...
    private fun dns(v2rayConfig: V2rayConfig): Boolean {
        try {
            ...
            val blkDomain = userRule2Domian(settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_V2RAY_ROUTING_BLOCKED)
                    ?: "")
            if (blkDomain.size > 0) {
