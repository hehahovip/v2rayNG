...
import com.dd.sie.service.V2RayTestService
...
object MessageUtil {
    ...
    fun sendMsg2Service(ctx: Context, what: Int, content: Serializable) {
        sendMsg(ctx, com.dd.sie.AppConfig.BROADCAST_ACTION_SERVICE, what, content)
    }

    fun sendMsg2UI(ctx: Context, what: Int, content: Serializable) {
        sendMsg(ctx, com.dd.sie.AppConfig.BROADCAST_ACTION_ACTIVITY, what, content)
    }
    ...
    private fun sendMsg(ctx: Context, action: String, what: Int, content: Serializable) {
        try {
            ...
            intent.`package` = com.dd.sie.AppConfig.ANG_PACKAGE
            intent.putExtra("key", what)
            intent.putExtra("content", content)
