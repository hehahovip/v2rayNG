...
import com.dd.sie.AppConfig.ANG_CONFIG
import com.dd.sie.AppConfig.HTTPS_PROTOCOL
import com.dd.sie.AppConfig.HTTP_PROTOCOL
import com.dd.sie.R
import com.dd.sie.dto.*
import com.dd.sie.dto.V2rayConfig.Companion.DEFAULT_SECURITY
import com.dd.sie.dto.V2rayConfig.Companion.TLS
import com.dd.sie.util.MmkvManager.KEY_SELECTED_SERVER
import java.net.URI
import java.util.*
import com.dd.sie.extension.idnHost
...
object AngConfigManager {
    ...
    fun migrateLegacyConfig(c: Context): Boolean? {
        try {
            ...
            val angConfig = Gson().fromJson(context, com.dd.sie.dto.AngConfig::class.java)
    ...
    private fun copyLegacySettings(sharedPreferences: SharedPreferences) {
        ...
        settingsStorage?.encode(
            ...
            sharedPreferences.getBoolean(com.dd.sie.AppConfig.PREF_SNIFFING_ENABLED, true)
        )
        settingsStorage?.encode(
            com.dd.sie.AppConfig.PREF_PER_APP_PROXY_SET,
            sharedPreferences.getStringSet(com.dd.sie.AppConfig.PREF_PER_APP_PROXY_SET, setOf())
        )
    }

    private fun migrateVmessBean(angConfig: com.dd.sie.dto.AngConfig, sharedPreferences: SharedPreferences) {
        angConfig.vmess.forEachIndexed { index, vmessBean ->
            val type = com.dd.sie.dto.EConfigType.fromInt(vmessBean.configType) ?: return@forEachIndexed
            val config = ServerConfig.create(type)
            config.remarks = vmessBean.remarks
            config.subscriptionId = vmessBean.subid
            if (type == com.dd.sie.dto.EConfigType.CUSTOM) {
                ...
                config.outboundBean?.settings?.vnext?.get(0)?.let { vnext ->
                    ...
                    if (config.configType == com.dd.sie.dto.EConfigType.VMESS) {
                        vnext.users[0].alterId = vmessBean.alterId
                        vnext.users[0].security = vmessBean.security
                    } else if (config.configType == com.dd.sie.dto.EConfigType.VLESS) {
                ...
                config.outboundBean?.settings?.servers?.get(0)?.let { server ->
                    ...
                    if (config.configType == com.dd.sie.dto.EConfigType.SHADOWSOCKS) {
                        server.password = vmessBean.id
                        server.method = vmessBean.security
                    } else if (config.configType == com.dd.sie.dto.EConfigType.SOCKS) {
                    ...
                    } else if (config.configType == com.dd.sie.dto.EConfigType.TROJAN) {
                        server.password = vmessBean.id
                    }
                }
                config.outboundBean?.streamSettings?.let { streamSetting ->
                    ...
                    val allowInsecure = if (vmessBean.allowInsecure.isBlank()) {
                        settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_ALLOW_INSECURE) ?: false
                    } else {
    ...
    private fun migrateSubItemBean(angConfig: com.dd.sie.dto.AngConfig) {
    ...
    private fun importConfig(
        ...
        try {
            ...
            val allowInsecure = settingsStorage?.decodeBool(com.dd.sie.AppConfig.PREF_ALLOW_INSECURE) ?: false
            if (str.startsWith(com.dd.sie.dto.EConfigType.VMESS.protocolScheme)) {
                config = ServerConfig.create(com.dd.sie.dto.EConfigType.VMESS)
                ...
                if (!tryParseNewVmess(str, config, allowInsecure)) {
                    if (str.indexOf("?") > 0) {
                        ...
                        var result = str.replace(com.dd.sie.dto.EConfigType.VMESS.protocolScheme, "")
            ...
            } else if (str.startsWith(com.dd.sie.dto.EConfigType.SHADOWSOCKS.protocolScheme)) {
                config = ServerConfig.create(com.dd.sie.dto.EConfigType.SHADOWSOCKS)
                if (!tryResolveResolveSip002(str, config)) {
                    var result = str.replace(com.dd.sie.dto.EConfigType.SHADOWSOCKS.protocolScheme, "")
            ...
            } else if (str.startsWith(com.dd.sie.dto.EConfigType.SOCKS.protocolScheme)) {
                var result = str.replace(com.dd.sie.dto.EConfigType.SOCKS.protocolScheme, "")
                val indexSplit = result.indexOf("#")
                config = ServerConfig.create(com.dd.sie.dto.EConfigType.SOCKS)
                if (indexSplit > 0) {
                    try {
                        config.remarks =
                            Utils.urlDecode(result.substring(indexSplit + 1, result.length))
            ...
            } else if (str.startsWith(com.dd.sie.dto.EConfigType.TROJAN.protocolScheme)) {
                val uri = URI(Utils.fixIllegalUrl(str))
                config = ServerConfig.create(com.dd.sie.dto.EConfigType.TROJAN)
            ...
            } else if (str.startsWith(com.dd.sie.dto.EConfigType.VLESS.protocolScheme)) {
                val uri = URI(Utils.fixIllegalUrl(str))
                val queryParam = uri.rawQuery.split("&")
                    .associate { it.split("=").let { (k, v) -> k to Utils.urlDecode(v) } }
                config = ServerConfig.create(com.dd.sie.dto.EConfigType.VLESS)
                val streamSetting = config.outboundBean?.streamSettings ?: return -1
                var fingerprint = streamSetting.tlsSettings?.fingerprint
    ...
    private fun tryResolveVmess4Kitsunebi(server: String, config: ServerConfig): Boolean {
        ...
        var result = server.replace(com.dd.sie.dto.EConfigType.VMESS.protocolScheme, "")
        val indexSplit = result.indexOf("?")
        if (indexSplit > 0) {
            result = result.substring(0, indexSplit)
        }
    ...
    private fun tryResolveResolveSip002(str: String, config: ServerConfig): Boolean {
        try {
            ...
            Log.d(com.dd.sie.AppConfig.ANG_PACKAGE, e.toString())
            return false
        }
    }
    ...
    private fun shareConfig(guid: String): String {
        try {
            ...
            return config.configType.protocolScheme + when (config.configType) {
                com.dd.sie.dto.EConfigType.VMESS -> {
                ...
                com.dd.sie.dto.EConfigType.SHADOWSOCKS -> {
                ...
                com.dd.sie.dto.EConfigType.SOCKS -> {
                    val remark = "#" + Utils.urlEncode(config.remarks)
                    val pw =
                ...
                com.dd.sie.dto.EConfigType.TROJAN -> {
                    val remark = "#" + Utils.urlEncode(config.remarks)

                    val dicQuery = HashMap<String, String>()
                    if (config.configType == com.dd.sie.dto.EConfigType.VLESS) {
     ...
     * upgrade
     */
    private fun upgradeServerVersion(vmess: com.dd.sie.dto.AngConfig.VmessBean): Int {
        try {
