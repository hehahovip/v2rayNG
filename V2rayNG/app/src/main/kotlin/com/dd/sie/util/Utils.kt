...
object Utils {
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_REMOTE_DNS) ?: com.dd.sie.AppConfig.DNS_AGENT
        val ret = remoteDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(com.dd.sie.AppConfig.DNS_AGENT)
        }
        return ret
    }

    fun getVpnDnsServers(): List<String> {
        val vpnDns = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_VPN_DNS)
                ?: settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_REMOTE_DNS)
                ?: com.dd.sie.AppConfig.DNS_AGENT
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns = settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_DOMESTIC_DNS) ?: com.dd.sie.AppConfig.DNS_DIRECT
        val ret = domesticDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(com.dd.sie.AppConfig.DNS_DIRECT)
        }
        return ret
    }
    ...
    fun stopVService(context: Context) {
        ...
        MessageUtil.sendMsg2Service(context, com.dd.sie.AppConfig.MSG_STATE_STOP, "")
    }
    ...
    fun userAssetPath(context: Context?): String {
        if (context == null)
            return ""
        val extDir = context.getExternalFilesDir(com.dd.sie.AppConfig.DIR_ASSETS)
                ?: return context.getDir(com.dd.sie.AppConfig.DIR_ASSETS, 0).absolutePath
    ...
    fun getLocale(context: Context): Locale =
        when (settingsStorage?.decodeString(com.dd.sie.AppConfig.PREF_LANGUAGE) ?: "auto") {
            "auto" ->  getSysLocale()
            "en" -> Locale("en")
            "zh-rCN" -> Locale("zh", "CN")
            "zh-rTW" -> Locale("zh", "TW")
